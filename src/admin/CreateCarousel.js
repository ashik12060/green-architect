...
const CreateCarousel = () => {
  ...
  const {
    ...
    validationSchema: validationSchema,
    onSubmit: (values, actions) => {
      createNewCarouselItem(values);
      actions.resetForm();
    },
  });

  const createNewCarouselItem = async (values) => {
    try {
      const { titleEn, titleBn, titleEs, image } = values;
      ...
      const result = await axiosInstance.post(`${process.env.REACT_APP_API_URL}/api/carousel/create`, data);
      if (result?.data?.success === true) {
      ...
      console.log(error);
      toast.error("Error adding carousel item. Please try again.");
    }
  };
    ...
    >
      ...
      <Box component="form" noValidate onSubmit={handleSubmit} sx={{ mt: 1 }}>
        ...
        <TextField
          ...
          fullWidth
          id="titleEn"
          label="Carousel Item Title (English)"
          name="titleEn"
          InputLabelProps={{
            shrink: true,
          }}
        ...
        <TextField
          ...
          fullWidth
          id="titleBn"
          label="Carousel Item Title (Bengali)"
          name="titleBn"
          InputLabelProps={{
            shrink: true,
          }}
        ...
        <TextField
          ...
          fullWidth
          id="titleEs"
          label="Carousel Item Title (Danish)"
          name="titleEs"
          InputLabelProps={{
            shrink: true,
          }}
