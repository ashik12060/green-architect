...
const EditProject = () => {
  ...
  const singleProjectById = async () => {
    ...
    try {
      // 
      const { data } = await axiosInstance.get(`${process.env.REACT_APP_API_URL}/api/project/${id}`
      );
  ...
  const updateProject = async (values) => {
    try {
      const result = await axiosInstance.put(`${process.env.REACT_APP_API_URL}/api/update/project/${id}`, values);
