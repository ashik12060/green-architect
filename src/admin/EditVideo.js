...
const EditPost = () => {
  ...
  const singlePostById = async () => {
    ...
    try {
      ...
      setFieldValue("title.en", data.video.title.en);
      setFieldValue("title.bn", data.video.title.bn);
      setFieldValue("title.es", data.video.title.es);
      setFieldValue(data.video.image.url);

      console.log("single post admin", data.video);
    } catch (error) {
      console.log(error);
      toast.error(error);
    }
  ...
  const updatePost = async (values) => {
    try {
      const result = await axiosInstance.put(
        `${process.env.REACT_APP_API_URL}/api/update/video/${id}`,
        values
      );
